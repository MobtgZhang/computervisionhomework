...
class FeaturePoints:
    def __init__(self,num_intervals=3, assumed_blur=0.5, image_border_width=5,sigma=1.6,float_tolerance = 1e-7):
    ...
    def compute_keypoints_descriptors(self,image):
        ...
        base_image = self.generate_base_image(image)
        num_octaves = self.compute_number_of_octaves(base_image.shape)
        gaussian_kernels = self.generate_gaussian_kernels()
